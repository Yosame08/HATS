...
with open("train_difDist_cnt.txt", "r") as f:
    ...
    for line in f:
        ...
        if len(info) <= 3:
            inter = int(info[0])
            data2[inter] = [0 for _ in range(int((statEnd - statBegin) / granularity) + 1)]
            continue
        for i in range(len(info)):
            val = - float(info[i]) / granularity
            if val >= 0 and val < statEnd / granularity:
...
plt.xlabel('Degree')
plt.ylabel('Sum')
plt.xlim([0, statEnd / granularity])
plt.ylim([0, 0.2])
x = np.arange(0, int(statEnd / granularity) + 1)
for key in data2:
    if key % 30 == 0: # show half
        continue
    val = key / 360
