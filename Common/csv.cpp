...
double CSVFile::get(int id, const string &header){
    if(headers.find(header)==headers.end())throw runtime_error(string("Wrong header: ")+header);
...
/**
 * Warning：You should not use the vector after calling this function
 */
void CSVFile::append(vector<double> &&line) {
    data.emplace_back(this);
    data.back().item = std::move(line);
